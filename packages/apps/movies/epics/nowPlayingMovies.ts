...
import action, { TAction } from '../actions/nowPlayingMovies'
...
export const nowPlayingMoviesEpic = (
  action$: ActionsObservable<TAction>
) => action$.pipe(
  filter(isActionOf(action.request)),
  switchMap(({ payload }) =>
    ajax.getJSON<TResponse>(`api/movies/now-playing?page=${payload.page}`).pipe(
      mergeMap((response) => {
